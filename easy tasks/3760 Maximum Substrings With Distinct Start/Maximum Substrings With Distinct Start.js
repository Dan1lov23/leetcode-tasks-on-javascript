...
function MaximumSubstringsWithDistinctStart(s) {
    ...
    for (let a = 0; a <= s.length; a++) {
        let actualElement = s[0];
        if (actualElement !== s[a] && !includeElement.includes(s[a])) {
            counter++;
            actualElement = s[a];
            includeElement.push(s[a]);
        }
    }
...
console.log(MaximumSubstringsWithDistinctStart("aaaa"))
